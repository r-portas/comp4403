 ...
 *               ReadStatement | WriteStatement | CompoundStatement | RepeatStatement
 ...
 *  LValue -> IDENTIFIER
 *  RepeatStatement -> KW_REPEAT Statement KW_UNTIL Condition
 *
...
public class Parser {
    ...
    private StatementNode parseStatement( TokenSet recoverSet ) {
        ...
        case KW_BEGIN:
            ...
            break;
        case KW_REPEAT:
            result = parseRepeatStatement( recoverSet );
            break;
        default:
    ...
    private StatementNode parseRepeatStatement( TokenSet recoverSet ) {
        tokens.beginRule("Repeat Statement", Token.KW_REPEAT);
        Location loc = tokens.getLocation();
        tokens.match( Token.KW_REPEAT );

        StatementNode s = parseStatementList( recoverSet.union(Token.KW_UNTIL) );
        tokens.match( Token.KW_UNTIL, CONDITION_START_SET );

        ExpNode cond = parseCondition( recoverSet );
        tokens.endRule( "Repeat Statement", recoverSet);

        return new StatementNode.RepeatNode(loc, s, cond);
    }
